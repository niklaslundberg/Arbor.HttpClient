...
With the Workflow/Coordinator extraction backlog (items 1–6) exhausted, the [Test split plan](#test-split-plan) done (2026-06-14, later 7), and **item 7 (DockFactory feature registrations) now done** (2026-06-14, later 11 — all four dockables plus the shared `VariableTextBox` decoupled via feature VMs / narrow context interfaces), the original "split MainWindowViewModel into separate features" goal is met. `MainWindowViewModel` remains the composition root that implements the per-panel context interfaces; further work (moving the panel state itself out of Main into dedicated feature VMs, or a registration-list `DockFactory`) is optional and incremental.
