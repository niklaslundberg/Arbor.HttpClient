...
**Progress so far (PR #208, commit `a6344d3`):**
- History entries are now clickable and load into the request editor (`LoadHistoryRequestCommand`), but immediate send is still missing.
...
**What it means:** OpenAPI import now enriches each imported request with structural metadata drawn directly from the spec: query parameters are appended to the URL as `{{param}}` placeholders, header parameters are stored on the request, default security/auth headers can be stored on the collection and inherited into each request (with per-request override/disable support), the first available example body is stored and populated on load, the matching `Content-Type` is detected and applied, and OpenAPI tags drive the tree-view grouping instead of path segment splitting.

**What shipped:**
- `CollectionRequest` gains four new optional fields: `Tag`, `Body`, `ContentType`, `Headers`
- `OpenApiImportService.Import` extracts: first operation tag → `Tag`; query parameters → appended to path as `?param={{param}}`; header parameters → `Headers` as `{{paramName}}`; HTTP Bearer / Basic / API Key security schemes → request-level `Headers` or collection-level default `Headers` (for document-level security); first JSON example (inline or named) → `Body`; content media type → `ContentType`
- `CollectionItemViewModel.GroupKey` now prefers `Tag` over the first path segment for tree grouping
- `LoadCollectionRequestCore` in `MainWindowViewModel` populates body/content type and merges collection default headers with request headers (request-level headers override by name; disabled request headers opt out inherited defaults)
- SQLite schema: `tag`, `body`, `content_type`, `headers` (request-level) and collection-level `headers` columns added via non-destructive `ALTER TABLE` migrations; headers serialised as JSON
