...
7. **DockFactory feature registrations** 🔶 Partial (2026-06-14, later 9) — replace the `MainWindowViewModel` constructor dependency for each dockable with a clean feature VM or a narrow per-feature interface. Two of the four dockables are now decoupled:
   ...
   - **`LeftPanelViewModel`** and **`RequestViewModel`** remain on `public MainWindowViewModel App { get; }` — their views bind ~40 and ~70 distinct `App.*` members respectively (collections/history/scheduled-jobs and the entire request/response/streaming surface). These are the genuinely large migrations and stay deferred, one view at a time.

   Verified headlessly: the screenshot E2E tests (`ScreenshotGenerator.CaptureScreenshot_LogPanel_SavesToDisk` / `CaptureScreenshot_LayoutPanel_SavesToDisk`) render the real views through `DockFactory` + compiled bindings; `log-panel.png` and `layout-panel.png` are byte-identical before/after. New focused tests: `DockableViewModelDecouplingTests`.

> **Status check (2026-06-14, later 9):** Item 7 is now in progress (2 of 4 dockables decoupled). The headless-verification blocker noted in earlier status checks is resolved — the existing `Category=Screenshots` E2E tests render each dockable through the real `DockFactory` and compiled bindings with no display server, so a binding regression fails the build (compiled-binding validation) or changes the rendered PNG. `DockFactory.CreateLayout()` no longer reaches through `_mainVm` for the Logs panel; `LeftPanelViewModel`/`RequestViewModel` still do. Recommendation for the remaining two: continue one view at a time, regenerating the relevant screenshot to confirm no visual regression.
...
- **Item 7 above** (DockFactory feature registrations) — partially done (Logs + Layout dockables decoupled); the `LeftPanelViewModel` and `RequestViewModel` dockable/view migrations remain, each verifiable via the `Category=Screenshots` E2E tests.
