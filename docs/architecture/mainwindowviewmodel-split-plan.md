...
| Collection-group expansion-state capture | `CollectionFilterWorkflow` | ✅ Done (2026-06-14, later 8) — moved the `CollectionGroups` → `GroupKey`/`IsExpanded` dictionary build from `ApplyCollectionFilter` to `CollectionFilterWorkflow.CaptureExpansionState(IEnumerable<CollectionGroupViewModel>)`, alongside `BuildCollectionItems`/`Apply`. `ApplyCollectionFilter` now just calls it before `_collectionFilterWorkflow.Apply`. Tests: new cases in `CollectionFilterWorkflowTests`. |
| Manual (non-inherited) header selection | `CollectionInheritedHeadersWorkflow` | ✅ Done (2026-06-14, later 8) — moved the `!IsInherited` filter from `SyncActiveCollectionRequestInheritedHeaders` to `CollectionInheritedHeadersWorkflow.SelectManualHeaders(IEnumerable<RequestHeaderViewModel>)`, alongside the other header helpers. `SyncActiveCollectionRequestInheritedHeaders` now just calls it. Tests: new cases in `CollectionInheritedHeadersWorkflowTests`. |
...
7. **DockFactory feature registrations** 🔶 Partial (2026-06-14, later 9) — replace the `MainWindowViewModel` constructor dependency for each dockable with a clean feature VM or a narrow per-feature interface. Two of the four dockables are now decoupled:
   - **`LogPanelViewModel`** ✅ — its view (`LogPanelView.axaml`) only ever bound `App.LogWindowViewModel.*`, so the dockable now takes `LogWindowViewModel` directly (`new LogPanelViewModel(logWindowViewModel)`); the view binds `Logs.*` and no longer references `MainWindowViewModel` at all. `DockFactory` receives the `LogWindowViewModel` as a constructor parameter alongside the existing `OptionsViewModel`/`EnvironmentsViewModel`/`CookieJarViewModel`.
   - **`LayoutManagementViewModel`** ✅ — its view bound only `App.SavedLayoutNames`/`App.SelectedLayoutName` plus four already-proxied commands, so the dockable now takes the narrow `ILayoutManagementContext` interface (`Features/Layout/`) implemented by `MainWindowViewModel` (the four command members via explicit interface implementation since the generated `ReactiveCommand<…>` properties don't match `ICommand` by return type). The `SelectedLayoutName` setter still routes back into Main's dock-tree pipeline by design.
   - **`LeftPanelViewModel`** ✅ (2026-06-14, later 10) — the Explorer dockable now takes the `ILeftPanelContext` interface (`Features/Collections/`) instead of `MainWindowViewModel`. The interface documents the panel's full surface: the History/Collections/Scheduled-Jobs tab state, the collection management commands and new/rename form state, the inherited-headers editor, the search/sort/display/grouping state, the scheduled-jobs list, and the two sub-VMs the item templates reach into (`RequestEditor` for well-known header names, `ResponseActions` for "Copy as cURL"). `MainWindowViewModel` implements it (properties implicitly; the `ReactiveCommand<…>` commands and `ActiveEnvironmentVariables` via explicit interface implementation). As part of this, the shared `VariableTextBox` control's `AppViewModel` property was retargeted from `MainWindowViewModel?` to the narrow `IVariableAutoCompleteHost?` (`Features/Variables/`, just `ActiveEnvironmentVariables`) so both the Explorer's inherited-header inputs and the request editor's variable inputs can supply it without depending on the whole VM.
   - **`RequestViewModel`** ✅ (2026-06-14, later 11) — the Request document now takes the `IRequestPanelContext` interface (`Features/HttpRequest/`) instead of `MainWindowViewModel`. This is the largest surface: the request-tab strip, the per-tab `RequestEditorViewModel` plus the GraphQL/WebSocket/SSE/scripting sub-VMs, the primary-action and demo-server state, the **integrated response panel** (`EmbeddedResponseView` binds the full response-state projection through `RequestViewModel.App`), and the UI font settings the embedded TextMate code editors read. Both code-behinds (`RequestView.axaml.cs`, `EmbeddedResponseView.axaml.cs`) had their `_appVm` field, `GetAppVm()`, `ApplyEditorFont`, and `nameof(MainWindowViewModel.X)` change-dispatch checks retyped to the interface (and dropped their `using Features.Main`). The standalone `ResponseView` (a separate dockable bound directly to `MainWindowViewModel`, not via `RequestViewModel`) is unaffected. `MainWindowViewModel` implements it (properties implicitly; the `ReactiveCommand<…>` commands via explicit interface implementation).

   Verified headlessly: the screenshot E2E tests render the real views through `DockFactory` + bindings; `log-panel.png`, `layout-panel.png`, `collections-panel.png`, `collections-panel-tree.png`, `scheduled-jobs.png`, `state-initial.png`, `request-tabs.png`, and the `variables-*` request-editor showcases are byte-identical before/after, and `state-response.png` renders the response body/status/action-buttons correctly (it differs only in the non-deterministic elapsed-time/timestamp text — `variables.png`/`main-window.png`/`state-response.png` are inherently run-to-run non-deterministic, confirmed by same-code re-runs). New focused tests: `DockableViewModelDecouplingTests`.

> **Status check (2026-06-14, later 11):** Item 7 is **done** — all four dockable VMs are decoupled from `MainWindowViewModel` (Logs → `LogWindowViewModel`; Layout → `ILayoutManagementContext`; Explorer → `ILeftPanelContext`; Request → `IRequestPanelContext`), plus the shared `VariableTextBox` control → `IVariableAutoCompleteHost`. `DockFactory` still receives `MainWindowViewModel` and passes it where a context interface is expected (Main implements all of them), but no dockable VM or its view code-behind references the concrete `MainWindowViewModel` type anymore. The headless-verification blocker from earlier status checks is fully resolved: every dockable is rendered through the real `DockFactory` by the `Category=Screenshots` E2E tests. A possible future step is to have `DockFactory` accept the context interfaces directly (rather than the concrete VM) and/or a per-feature registration list, but that is now a small, mechanical change rather than a coupling problem.

> **Update 2026-06-14 (later 12):** Completed the optional `DockFactory` cleanup noted above. `DockFactory`'s constructor now takes `ILayoutManagementContext`, `ILeftPanelContext`, and `IRequestPanelContext` directly (plus the existing `EnvironmentsViewModel`/`OptionsViewModel`/`CookieJarViewModel`/`LogWindowViewModel`), instead of the concrete `MainWindowViewModel`; `CreateLayout()` constructs `LeftPanelViewModel`/`LayoutManagementViewModel`/`RequestViewModel` from these typed fields, and dropped its `using Features.Main`. `MainWindowViewModel`'s constructor passes `this` three times (once per interface, via implicit conversion) at the `new DockFactory(...)` call site. Verified headlessly: `layout-panel.png`, `log-panel.png`, `collections-panel.png`, `collections-panel-tree.png`, `scheduled-jobs.png`, `state-initial.png`, `request-tabs.png`, and the `variables-*` screenshots are byte-identical before/after; `dotnet test Arbor.HttpClient.slnx` still reports 869/869 passing.

#### Remaining work beyond this plan

With the Workflow/Coordinator extraction backlog (items 1–6) exhausted, the [Test split plan](#test-split-plan) done (2026-06-14, later 7), and **item 7 (DockFactory feature registrations) now done** (2026-06-14, later 11/12 — all four dockables plus the shared `VariableTextBox` decoupled via feature VMs / narrow context interfaces, and `DockFactory` itself now depends only on those interfaces and feature VMs, not on `MainWindowViewModel`), the original "split MainWindowViewModel into separate features" goal is met. `MainWindowViewModel` remains the composition root that implements the per-panel context interfaces; further work (moving the panel state itself out of Main into dedicated feature VMs, or a registration-list `DockFactory`) is optional and incremental.

> **Update 2026-06-14 (later 13):** Started on the "move panel state out of Main" follow-up, taking the smallest of the three remaining dockables (Layout). `LayoutManagementViewModel` (`Features/Layout/`) now owns the saved-layout state (`SavedLayoutNames`, `SelectedLayoutName`) and the four layout commands (`SaveLayoutAsNewCommand`/`SaveLayoutToExistingCommand`/`RestoreDefaultLayoutCommand`/`RemoveLayoutCommand`) directly, including the `_layoutWorkflow`-backed `LoadFromOptions`/`BuildNamedLayouts` and the `WhenAnyValue(SelectedLayoutName)`-driven restore subscription. It takes only the five dock-tree/window-geometry delegates from Main (`refreshDockTreeCache`, `captureLayoutSnapshot`, `applyLayoutSnapshot`, `persistLayoutOptions`, `getDefaultLayout`), per the "Apply* UI projections stay in Main" precedent. `ILayoutManagementContext` is deleted; `MainWindowViewModel` no longer implements it and no longer owns `_layoutWorkflow`/`_suppressLayoutRestore`/`SelectedLayoutName`/the four layout commands — these are reached via the new `MainWindowViewModel.LayoutManagement` property. `DockFactory` now takes the `LayoutManagementViewModel` instance directly (same pattern as `LogPanelViewModel`/`LogWindowViewModel`) instead of constructing it from a context interface. `MainWindowViewModel` is down to about 2,527 lines. Tests: `DockableViewModelDecouplingTests.LayoutManagementViewModel_OwnsSavedLayoutStateAndCommands` replaces the old context-proxy test; `MainWindowLayoutUiTests` updated to go through `viewModel.LayoutManagement.*`. `dotnet test Arbor.HttpClient.slnx` reports 869/869 passing. The Explorer (`ILeftPanelContext`) and Request (`IRequestPanelContext`) slices remain as future, larger increments of this same follow-up.
