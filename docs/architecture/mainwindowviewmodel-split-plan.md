...
7. **DockFactory feature registrations** 🔶 Partial (2026-06-14, later 9) — replace the `MainWindowViewModel` constructor dependency for each dockable with a clean feature VM or a narrow per-feature interface. Two of the four dockables are now decoupled:
   ...
   - **`RequestViewModel`** remains on `public MainWindowViewModel App { get; }` — its view binds ~70 distinct `App.*` members (the entire request/response/streaming/scripting surface). This is the last and largest migration and stays deferred.

   Verified headlessly: the screenshot E2E tests render the real views through `DockFactory` + bindings; `log-panel.png`, `layout-panel.png`, `collections-panel.png`, `collections-panel-tree.png`, `scheduled-jobs.png`, and the `variables-*` request-editor showcases are byte-identical before/after (the `variables.png`/`main-window.png`/`state-response.png` screenshots are inherently run-to-run non-deterministic — confirmed by same-code re-runs — due to async environment-load/response-timing, unrelated to these changes). New focused tests: `DockableViewModelDecouplingTests`.

> **Status check (2026-06-14, later 10):** Item 7 is now mostly done (3 of 4 dockables decoupled: Logs, Layout, Explorer). The headless-verification blocker noted in earlier status checks is resolved — the existing `Category=Screenshots` E2E tests render each dockable through the real `DockFactory` with no display server, so a binding regression either fails the build (compiled-binding validation, where enabled) or changes a deterministic rendered PNG. Only `RequestViewModel` still takes the full `MainWindowViewModel`; its `RequestView.axaml` (~70 `App.*` bindings, `x:CompileBindings="False"`) is the remaining migration. Recommendation: introduce an `IRequestPanelContext` mirroring the `ILeftPanelContext` pattern, verifying against `state-initial.png`/`state-response.png`/`request-tabs.png`/`variables-*.png`.

#### Remaining work beyond this plan

With the Workflow/Coordinator extraction backlog (items 1–6) essentially exhausted — further rounds now find only marginal pure-helper moves like the `Method` projection above — and the [Test split plan](#test-split-plan) now done (2026-06-14, later 7), the work left for "split MainWindowViewModel into separate features" is:

- **Item 7 above** (DockFactory feature registrations) — mostly done (Logs + Layout + Explorer dockables decoupled, plus the shared `VariableTextBox` control); only the `RequestViewModel` dockable/view migration remains, verifiable via the `Category=Screenshots` E2E tests.
